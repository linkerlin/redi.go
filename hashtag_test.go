...
// redisSlot mirrors Redis Cluster HASH_SLOT (CRC16-CCITT mod 16384) with
// hash-tag extraction — same algorithm as go-redis/internal/hashtag.
...
// CRC16-CCITT table from Redis Cluster spec / go-redis hashtag.
...
func crc16sum(key string) (crc uint16) {
	for i := 0; i < len(key); i++ {
		crc = (crc << 8) ^ crc16tab[(byte(crc>>8)^key[i])&0xff]
	}
	return
}
