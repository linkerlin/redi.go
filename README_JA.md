...
**Go ↔ 実 Redisson 4.6.1 の双方向相互運用テスト**（51 個の `TestJavaInterop_*`）および Go ↔ redi.py 双方向回帰（Multimap 含む）を通過済みです。`Client.Get*` ファクトリは現在 **66 種のユニークな R\* 戻り値型**をカバーします。貢献方法は [CONTRIBUTING.md](CONTRIBUTING.md) を参照してください。
