...
已通过 **Go ↔ 真实 Redisson 4.6.1 直接双向互操作测试**（54 个 `TestJavaInterop_*` 测试函数）与 Go ↔ redi.py 双向回归（含 Multimap 等）。`Client.Get*` 工厂当前覆盖 **66 种唯一 R* 返回类型**。贡献指南见 [CONTRIBUTING.md](CONTRIBUTING.md)。
